...
def get_arguments():
	...
	parser.add_argument("--filter-scale", type=int, default=1,
						...
						choices=[1, 2])
	parser.add_argument("--use-quantize", action="store_true",
						help="whether to use quantize.")
...
def main():
	...
	# try quantize to uint8
	if args.use_quantize:
		print("evaluation with quantizing to int8...")
		is_training = False
		g = tf.get_default_graph()
		if is_training:
			tf.contrib.quantize.create_training_graph(g)
		else:
			...
			if os.path.exists("quantize_graph"):
				pass
				#os.remove("quantize/*")
			else:
				os.makedirs("quantize_graph")
			with open("./quantize_graph/eval_graph.txt", "w") as f:
				f.write(str(g.as_graph_def()))
			#sess = tf.Session()
			saver = tf.train.Saver()
			saver.save(sess, "./quantize_graph")
			exit(0)
	print("This point is not available now.")
	assert False
