...
def main():
    ...
    raw_output_up = tf.image.resize_bilinear(raw_output, size=(1024,2048), align_corners=True)
    ...
    for i in trange(param['num_steps'], desc=description, leave=True):
        ...
        input_img, input_lbl = sess.run([img, anno], feed_dict = {image_filename: img_files[i], anno_filename: anno_files[i]})

        feed_dict = {input_image: input_img, input_label: input_lbl}
