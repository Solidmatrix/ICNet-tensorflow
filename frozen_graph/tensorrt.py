...
def preprocess_image(file_name, output_height=224, output_width=224,
  ...
  #     is_training=False)
  tf.cast(image_buffer, dtype=tf.float32)
  with tf.Session(config=get_gpu_config()) as sess:
    result = sess.run([image_buffer])

  return result[0]


def batch_from_image(file_name, batch_size, output_height=1025, output_width=2049,
                     num_channels=3):
...
def batch_from_random(batch_size, output_height=1025, output_width=2049,
                      num_channels=3):
...
def time_graph(graph_def, data, input_node, output_node, num_loops=100):
  ...
  with g.as_default():
    ...
    img = misc.imread('/home/medialab/yilun/pspnet+icnet/ICNet-tensorflow/input/outdoor_1.png', mode='RGB').astype(np.float32)
    img = tf.expand_dims(img, axis=0)
    return_tensors = tf.import_graph_def(
        graph_def=graph_def,
        input_map={input_node: img},
        # input_map={input_node: iterator.get_next()},
        # input_map={input_node: '/home/gwy/tensorflow/workSpace/ICNet-tensorflow/input/outdoor_1.jpg'},
  ...
  with tf.Session(graph=g, config=get_gpu_config()) as sess:
    ...
    for _ in range(num_loops):
      ...
      print('average inference time: {}'.format(np.mean(time_list[-100:])))
      print('frame per second: {}'.format(1.0 / np.mean(time_list[-100:])))
